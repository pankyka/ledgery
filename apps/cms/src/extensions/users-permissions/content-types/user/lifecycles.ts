...
export default factories.createCoreController('plugin::users-permissions.user', ({ strapi }) =>  ({
  async afterCreate(event) {
    ...
    const timeSuffix = new Date().toLocaleTimeString('hu-HU', { hour: '2-digit', minute: '2-digit' }).replace(':', '');
    ...
    const newTenant = await strapi.entityService.create('api::tenant.tenant', {
